...
if data and company_name:
    ...
    if len(output) > 0:
        st.subheader(output.iloc[0]['사업장명'])   # 검색결과 첫번째 사업장명을 '제목'으로 사용.
        ...
        with col1:
            st.text('월급여 추정')
            st.markdown(f"`{int(output.iloc[0]['월급여추정']):,}` 원")

        with col2:
            st.text('연봉 추정')
            st.markdown(f"`{int(output.iloc[0]['연간급여추정']):,}` 원")

        with col3:
            st.text('가입자수 추정')
            st.markdown(f"`{int(output.iloc[0]['가입자수']):,}` 명")
